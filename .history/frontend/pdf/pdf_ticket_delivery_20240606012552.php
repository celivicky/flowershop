<?php
   ...
   # Incluyendo librerias necesarias #
    ...
    $stmt = $connect->prepare("SELECT * FROM configuracion");
$stmt->setFetchMode(PDO::FETCH_ASSOC);
$stmt->execute();
while($a = $stmt->fetch()){

    $pdf->MultiCell(0,5,utf8_decode(strtoupper($a['nomemp'])),0,'C',false);
    $pdf->SetFont('Arial','',9);
    $pdf->MultiCell(0,5,utf8_decode("RIF: ".$a['rucem']),0,'C',false);
    $pdf->MultiCell(0,5,utf8_decode("Direccion: ".$a['direcc']),0,'C',false);
    $pdf->MultiCell(0,5,utf8_decode("Teléfono:".$a['teleem']),0,'C',false);
    ...
    $pdf->MultiCell(0,5,utf8_decode("Fecha:".date("d/m/Y") ),0,'C',false);
     ...
     $id = $_GET['id'];
    $stmt = $connect->prepare("SELECT orders.method,orders.idord, orders.user_id, clientes.idclie, clientes.nomclie, clientes.apeclie,clientes.telclie, clientes.correo, orders.total_products, orders.total_price, orders.placed_on, orders.payment_status, orders.tipc, orders.ourentre, orders.tiemp, orders.distri, orders.direcenvio, orders.refer, orders.deen, orders.paren, orders.mendedi, orders.foto FROM orders INNER JOIN clientes ON orders.user_cli = clientes.idclie WHERE orders.idord= '$id'");
$stmt->setFetchMode(PDO::FETCH_ASSOC);
$stmt->execute();
while($row = $stmt->fetch()){
    ...
    $pdf->MultiCell(0,5,utf8_decode("Cliente:". $row['nomclie'] ."\n". $row['apeclie']),0,'C',false);

    $pdf->MultiCell(0,5,utf8_decode("Telefono:". $row['telclie']),0,'C',false);
    ...
    $pdf->Cell(32,5,utf8_decode("0.00"),0,0,'C');
    //$pdf->Cell(32,5,utf8_decode("S/".$row['total_price']),0,0,'C');
    ...
    $pdf->Cell(32,5,utf8_decode("$/".$row['total_price']),0,0,'C');
